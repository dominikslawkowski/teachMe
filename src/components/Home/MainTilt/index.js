...
class MainTilt extends React.Component {
    ...
    displayOffer() {
        return this.props.theUserData.oferuje.map((item) => {
            return <div className={"maciek-middle_tag-itself1"}>{item}</div>
        });
    }

    displaySearch() {
        return this.props.theUserData.szukam.map((item) => {
            return <div className={"maciek-middle_tag-itself2"}>{item}</div>
        });
    }

    displayName(){
        return this.props.theUserData.imie
    }

    displayAge() {
        return this.props.theUserData.wiek
    }

    displayDescription() {
        return this.props.theUserData.opis;
    }
